...
import GalleryView from '../../../components/ItemGallery';
import {getListInfo} from '../../../api/ProductAPI';
import {getListSize} from '../../../api/SizeAPI';
import SizeView from '../../../components/ItemSize';
...
const Details = ({route}) => {
  ...
  const addProductToCart = async () => {
    if (!product._id || !selectedGallery || !selectedSize || !selectedImage) {
      Alert.alert('Vui lòng chọn đủ thông tin trước khi thêm vào giỏ hàng.');
      return;
    }
    try {
      setLoading(true);
        ...
        <View style={styles.containerContent}>
          ...
          <Text style={styles.price}>
            {product.priceInitial
              ? product.priceInitial.toLocaleString('vi-VN') + ' đ'
              : 'Đang cập nhật'}
          </Text>
