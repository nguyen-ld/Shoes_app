...
import CartView from '../../../components/ItemCart';
...
const Cart = ({navigation, route, id_user}) => {
  ...
  const addNewOrder = async () => {
    ...
    try {
      ...
      console.log('Lỗi khi đặt hàng:', error.response?.data || error.message);
        ...
        <View style={styles.containerCheckout}>
            ...
            <View style={{marginBottom: 15}}>
              <View>
                <Text style={styles.infoUser}>
                  {id_address.fullname} | <Text>{id_address.numberphone}</Text>
                </Text>
                <Text style={styles.infoUser}>
                  {id_address.street} , Phường <Text>{id_address.ward}</Text> ,
                  Quận <Text>{id_address.district}</Text> ,{' '}
          ...
          {cart.length === 0 ? (
            <TouchableOpacity
              ...
              <Text style={styles.textButtonEnable}>Checkout</Text>
            </TouchableOpacity>
          ) : (
            <TouchableOpacity
              activeOpacity={0.9}
